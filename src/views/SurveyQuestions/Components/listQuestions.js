...
import infoTextSurvey from "./infoTextSurvey";
import TextAreaSurvey from "./textAreaSurvey";
import { BrowserView, MobileView } from "react-device-detect";

const ListQuestions = ({ data, objInfo, onBtnSaveClick, onBtnBack }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  var listScale = data.filter(function (x) {
    return x.name === "Scale";
  });
  var listQuestions = data.filter(function (x) {
    return x.name === "Questions";
  });
  var listQuestionsAnswer = data.filter(function (x) {
    return x.name === "QuestionsAnswer";
  });

  const renderQuestion = (objQuestion) => {
    switch (objQuestion.codTipoPregunta) {
      case 1:
        return dropDownSurvey(
          objQuestion,
          listScale,
          listQuestionsAnswer,
          register
      ...
      case 2:
        return TextAreaSurvey(objQuestion,listQuestionsAnswer, register);
      case 3:
        return infoTextSurvey(objQuestion);
    }
  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-between rounded-xl w-full shadow-2xl shadow-cyan-500/30 sha p-5 mb-12 md:flex-col-reverse md:gap-6 lg:p-14 lg:pt-10">
        <div className="mb-12">
          <MobileView>
            <div onClick={() => {
                      onBtnBack();
                    }} className="flex justify-center items-center w-[40px] h-[40px] bg-slate-100 rounded-full rounded-tl-xl hover:bg-slate-400 hover:text-slate-50 md:hidden">
              <svg
          ...
          <form onSubmit={handleSubmit((data) => onBtnSaveClick(data))}>
            <div className="flex flex-col gap-5">
              <h1 className="text-YVmagenta text-lg font-bold mt-3 text-center md:text-left">
                Por favor responda las siguientes preguntas{" "}
              </h1>
              <div className="group">
              {listQuestions[0].value.map((objQuestion) =>
                renderQuestion(objQuestion)
              )}
              </div>
              ...
              <BrowserView>
                <div className="mt-16 hidden flex-col justify-center items-center gap-3 md:flex">
                  ...
                  <a
                    ...
                    href="#"
                    className="hover:text-YVmagenta hover:underline underline-offset-2"
                  >
                    Regresar
                  </a>
                </div>
              ...
              <MobileView>
                <div className="mt-16 flex-col justify-center items-center gap-3 md:flex">
                  <Button>Guardar</Button>
                  <a
                    onClick={() => {
                      onBtnBack();
        ...
        <div className="flex flex-col justify-center items-center gap-1 md:flex-row-reverse md:justify-between">
          <div className="text-sm md:bg-cyan-50 rounded-full px-4 pt-1 pb-1">
            {objInfo.actualPage} / {objInfo.totalPage}
          </div>
          <div className="relative w-full bg-cyan-50 rounded-md px-3 pt-1 pb-2 md:w-9/12 items-center gap-4 md:flex md:rounded-full md:px-5">
            <div className="flex justify-between items-center">
              <p className="w-max">Progreso</p>
              <p className="font-bold text-xl text-slate-700 md:hidden">
                {objInfo.advance}%
              </p>
            </div>
            <div className="bg-white rounded-full mt-1 overflow-hidden w-full">
              <div
                className="bg-gradient-to-r from-YVmagenta via-YVyellow to-YVcyan h-2 rounded-full"
                style={{ width: objInfo.advance + "%" }}
              ></div>
            </div>
            <p className="font-bold text-xl text-slate-700 w-max hidden md:block">
              {objInfo.advance}%
            </p>
            {/* Alert */}
            <div className="hidden bg-black/80 w-[220px] leading-5 text-white p-3 rounded-xl items-center gap-2 absolute top-0 -mt-20 left-1/2 -translate-x-1/2">
